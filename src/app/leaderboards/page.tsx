...
export default function LeaderboardsPage() {
  ...
  const handleSearch = async (searchUsername: string) => {
    ...
    try {
      const response = await fetch(`/api/stats?username=${searchUsername}`);
          ...
          {leaderboards && (
            <div className="bg-gray-900/50 rounded-lg p-6 backdrop-blur-sm border border-gray-700/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Leaderboard
                  title="Total Commits"
                  entries={leaderboards.commits}
                  metric="commits"
                />
                <Leaderboard
                  title="Longest Streak"
                  entries={leaderboards.streak}
                  metric="days"
                />
                <Leaderboard
                  title="GitHub Stars"
                  entries={leaderboards.stars}
                  metric="stars"
                />
              </div>
            </div>
